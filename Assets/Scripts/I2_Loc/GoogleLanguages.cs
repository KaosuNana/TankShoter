...
namespace I2.Loc
{
	public static class GoogleLanguages
	{
		...
		public static string GetLanguageCode(string Filter, bool ShowWarnings = false)
		{
			...
			foreach (KeyValuePair<string, GoogleLanguages.LanguageCodeDef> current in GoogleLanguages.mLanguageDef)
			{
				if (GoogleLanguages.LanguageMatchesFilter(current.Key, filters))
				{
		...
		public static List<string> GetLanguagesForDropdown(string Filter, string CodesToExclude)
		{
			...
			foreach (KeyValuePair<string, GoogleLanguages.LanguageCodeDef> current in GoogleLanguages.mLanguageDef)
			{
				if (string.IsNullOrEmpty(Filter) || GoogleLanguages.LanguageMatchesFilter(current.Key, filters))
				{
		...
		public static string GetGoogleLanguageCode(string InternationalCode)
		{
			foreach (KeyValuePair<string, GoogleLanguages.LanguageCodeDef> current in GoogleLanguages.mLanguageDef)
			{
				if (InternationalCode == current.Value.Code)
				{
					return string.IsNullOrEmpty(current.Value.GoogleCode) ? InternationalCode : current.Value.GoogleCode;
				}
			}
		...
		public static List<string> GetAllInternationalCodes()
		{
			...
			foreach (KeyValuePair<string, GoogleLanguages.LanguageCodeDef> current in GoogleLanguages.mLanguageDef)
			{
				hashSet.Add(current.Value.Code);
			}
