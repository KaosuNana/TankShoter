...
public class MissionWaveCleared : Mission
{
	private void Start()
	{
	...
	public override void LoadMissionProgress()
	{
		...
		{
			ES2.Load<MissionWaveCleared>(this.savedFileName, this);
		}
		else
		{
			base.fileVersion = 1f;
	...
	{
		EventManager.StartListening("WaveCleared", new UnityAction(this.AddAmount));
	}

	private void SaveMissionProgress()
	{
		ES2.Save<MissionWaveCleared>(this, this.savedFileName);
	}

	public override void StopListening()
	{
		EventManager.StopListening("WaveCleared", new UnityAction(this.AddAmount));
	}
